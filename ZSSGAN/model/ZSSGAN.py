...
class ZSSGAN(torch.nn.Module):
    ...
    def determine_opt_layers(self):
        z_dim    = 64 if self.args.sgxl else 512
        # shape (8, 512)   Batch size for use in automatic layer selection step
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            # w_styles.shape (1, 2, 512)
            # val= torch.tensor([item.cpu().detach().numpy() for item in w_styles]).cuda()
            # print(val.shape)
            # frozen_img.shape(2, 3, 1024, 1024)
