...
def train(args):
    ...
    sample_dir = Path(os.path.join(args.output_dir, "sample"))
    ckpt_dir   = Path(os.path.join(args.output_dir, "checkpoint"))
    ...
    for i in tqdm(range(args.iter)):
        ...
        net.train()
        # shape (1, 2, 512)  sample_z  (1, batch, z_dim)
        sample_z = mixing_noise(args.batch, z_dim, args.mixing, device)
        # shape (2, 3, 1024, 1024)   shape (2, 3, 1024, 1024)
        [sampled_src, sampled_dst], loss = net(sample_z)
        # print(sampled_src.shape, sampled_dst.shape)
    ...
    for i in range(args.num_grid_outputs):
        ...
        with torch.no_grad():
            ...
            # val= torch.tensor([item.cpu().detach().numpy() for item in sample_z]).cuda()
            # print(val.shape)
